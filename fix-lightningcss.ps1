...
Write-Host "🗑️  Removing old Next.js image..." -ForegroundColor Yellow
docker rmi test-ptimp-nextjs -f 2>$null

# Rebuild without cache (important for installing build dependencies)
Write-Host "🏗️  Rebuilding Next.js with build tools (this may take 5-10 minutes)..." -ForegroundColor Yellow
...
if ($logs -match "Ready in") {
    Write-Host "✅ Next.js is running successfully!" -ForegroundColor Green
} elseif ($logs -match "Error|Cannot find module") {
    Write-Host "⚠️  There might be errors. Check logs with:" -ForegroundColor Yellow
    Write-Host "   docker-compose logs -f nextjs" -ForegroundColor Gray
} else {
    Write-Host "⏳ Next.js is still starting. Check logs in a moment." -ForegroundColor Yellow
}
